<%-- 
    Document   : updateunits_highcouncil
    Created on : 15 Jan 2025, 8:05:14 pm
    Author     : HP
--%>
...
<%
    // Periksa apakah sesi memiliki atribut highCouncil
    ...
    // Ambil objek highCouncil dari sesi
...
<html>
<head>
    <title>Update Units High Council</title>
    <style>
          body {
...
<body>
    ...
    <div id="sidebar">
        ...
        <a href="index.jsp">Logout</a>
            ...
            <div id="content">
    <h1>Update Information (Units High Council)</h1>
    <form action="updateController" method="post" onsubmit="return validateForm()">
        <label>Full Name:</label>
        <input type="text" name="name" value="<%= highCouncil.getName() %>" required>

        <label>Email:</label>
        <input type="text" name="email" value="<%= highCouncil.getEmail() %>" required readonly>

        <label>Squad:</label>
        <select name="squad" required>
            <option value="SQ16" <%= highCouncil.getSquad().equals("SQ16") ? "selected" : "" %>>SQ16</option>
            <option value="SQ17" <%= highCouncil.getSquad().equals("SQ17") ? "selected" : "" %>>SQ17</option>
            <option value="SQ18" <%= highCouncil.getSquad().equals("SQ18") ? "selected" : "" %>>SQ18</option>
        </select>
        
        <label>Position:</label>
        <select name="position" required>
            <option value="President" <%= highCouncil.getPosition().equals("President") ? "selected" : "" %>>President</option>
            <option value="Secretary" <%= highCouncil.getPosition().equals("Secretary") ? "selected" : "" %>>Secretary</option>
            <option value="Treasurer" <%= highCouncil.getPosition().equals("Treasurer") ? "selected" : "" %>>Treasurer</option>
        </select>

        <div class="button-group">
            <button type="submit">Save</button>
            <button type="button" onclick="location.href='dashboardunits_highcouncil.jsp'">Back</button>
        </div>
    </form>
            </div><!-- comment -->
               <footer>
